...
import mimetypes
import time
#from ocscripts import conf as MUSERConf
from api import *
import kafkaUtils
    ...
    def operation(self):
        try :
            ...
            if action == "add" :
                ...
                for v in num:
                    id = id+1
                    #MySQLConn.get().db.insert(tableName, refTime=refTime, theValue=v,freq = formFreq)
                if id ==5:


                    MySQLConn.get().db.insert(tableName, refTime=refTime, theValue=theValue,freq = formFreq)
                else:
                    return titled_render().error(error ='some error in your data')
            ...
            if action == "edit" :
                ...
                for v in num:
                    id = id+1
                if id ==5:
                    MySQLConn.get().db.update(tableName,where="id="+identify,  refTime=refTime, theValue=theValue,freq = formFreq)
                else:
...
class IntegrationResults(object):
    def GET(self):
        try :
            ...
            return JsonUtil.successMsgJson("")
        except Exception, e:
            traceback.print_exc()
...
class IntegrationList(object):
    def GET(self):
        try :
            ...
            return titled_render().integration(items = items,beginTime = beginTime,endTime=endTime)
        except Exception, e:
            return titled_render().error(error=e.message)
